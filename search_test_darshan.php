...
$con= mysqli_connect("localhost","root","3rdmarch","real_estate");
 ...
 if (mysqli_connect_errno($con))
 {
  ...
  $query = "select * from employees where ";
   ...
   {
	 $query=mysqli_query($con,$query." emp_id =".$_POST['search_by_empid']." and");
     	 
   }
  
  if(!empty($_POST['search_by_fname']))
   {
	 $query=mysqli_query($con,$query." first_name LIKE ".$_POST['search_by_fname']."% and"); 
   }
  
  if(!empty($_POST['search_by_lname']))
   {
	 $query=mysqli_query($con,$query." last_name LIKE ".$_POST['search_by_lname']."% and"); 
   }
   
   if(!empty($_POST['search_by_address']))
   {
	 $query=mysqli_query($con,$query." address LIKE ".$_POST['search_by_address']."% and"); 
   }
   
   if(!empty($_POST['search_by_contact_no']))
   {
	 $query=mysqli_query($con,$query." contact_no = ".$_POST['search_by_contact_no']." and"); 
   }
   
   if(!empty($_POST['search_by_email']))
   {
	 $query=mysqli_query($con,$query." email LIKE ".$_POST['search_by_email']."% and"); 
   }
  
   if(!empty($_POST['search_by_joining_date']))
   {
	 $query=mysqli_query($con,$query." joining_date >= ".$_POST['search_by_joining_date']." and");	 
   }
   
   if(!empty($_POST['search_by_salary']))
   {
	 $query=mysqli_query($con,$query." basic_salary = ".$_POST['search_by_salary']." and"); 
   }
